...
async def save_nutrient_solution_endpoint(request: Request) -> dict:
    ...
    for key, value in solution.targets_mg_per_l.items():
        ...
        try:
            ...
            raise HTTPException(status_code=400, detail=f"Invalid target value for {key}") from exc
...
@app.post("/recipes")
@app.put("/recipes")
async def save_recipe_endpoint(request: Request) -> dict:
    content_type = (request.headers.get("content-type") or "").lower()
    raw_body = await request.body()
    if "yaml" in content_type:
        payload = yaml.safe_load(raw_body.decode("utf-8")) or {}
    ...
    fertilizers = [{"name": entry.name, "grams": float(entry.grams)} for entry in recipe.fertilizers]
    water_profile = recipe.water_profile.strip() if recipe.water_profile else None
    if water_profile and water_profile.endswith(".yml"):
        water_profile = water_profile[: -len(".yml")]

    osmosis_percent = recipe.osmosis_percent if recipe.osmosis_percent is not None else 0
    try:
        osmosis_percent = float(osmosis_percent)
    except (TypeError, ValueError) as exc:
        raise HTTPException(status_code=400, detail="Invalid osmosis_percent value") from exc
    ...
    if water_profile:
        payload_out["water_profile"] = water_profile
    if osmosis_percent:
        payload_out["osmosis_percent"] = osmosis_percent
